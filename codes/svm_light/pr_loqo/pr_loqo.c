	    ...
	    double margin, double bound, int restart) 
{
  ...
  if (restart == 1) {
    ...
    for (i=0; i<n; i++) {	/* compute g, t for primal feasibility */
      g[i] = max1(ABS(x[i] - l[i]), bound);
      t[i] = max1(ABS(u[i] - x[i]), bound); 
    }
    ...
    for (i=0; i<n; i++) {
      g[i] = max1(ABS(x[i] - l[i]), bound);
      z[i] = max1(ABS(x[i]), bound);
      t[i] = max1(ABS(u[i] - x[i]), bound); 
      s[i] = max1(ABS(x[i]), bound); 
    }
  }
  ...
  while (status == STILL_RUNNING) {
	...
	delta_t[i] = t[i] * (gamma_s[i] - delta_s[i]) / s[i];
      ...
      alfa = -1;
      for (i=0; i<n; i++) {
	alfa = min1(alfa, delta_g[i]/g[i]);
	alfa = min1(alfa, delta_t[i]/t[i]);
	alfa = min1(alfa, delta_s[i]/s[i]);
	alfa = min1(alfa, delta_z[i]/z[i]);
      }
      alfa = (margin - 1) / alfa;
