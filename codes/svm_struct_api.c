...
void        print_struct_testing_stats(FILE *fp, SAMPLE sample, STRUCTMODEL *sm,
				       STRUCT_LEARN_PARM *sparm, 
				       STRUCT_TEST_STATS *teststats)
{
  ...
  fprintf(fp, "\nSummary:\n");
  fprintf(fp, "Runtime (without IO) in cpu-seconds: %.2f\n", (float)(teststats->runtime/100.0));
  fprintf(fp, "Precision on test set: %.2f%% (%ld correct, %ld incorrect, %d total)\n",100.0f*teststats->correct/sample.n,teststats->correct,teststats->incorrect,sample.n);
  fprintf(fp, "Precision and relative information on each classes (total %d classes):\n", teststats->num_classes);
  fprintf(fp, "Class(N)  Total(N)  Correct(N)  Incorrect(N)  MissAs(N)  Precision\n");
  for (i=0; i<teststats->num_classes; i++)
  {
  	fprintf(fp, "Class%-5.1d%-10.1d%-12.1d%-14.1d%-11.1d%.2f%%\n", i+1, 
		teststats->class_corrects[i]+teststats->class_incorrects[i],
		teststats->class_corrects[i], teststats->class_incorrects[i], 
		teststats->class_misses[i], 
		(float)teststats->class_corrects[i]/(teststats->class_corrects[i]+teststats->class_incorrects[i]));
  }
}
			    ...
			    STRUCT_TEST_STATS *teststats)
{
  ...
  if (exnum == 0) { /* this is the first time the function is
	...
	teststats->num_classes = sparm->num_classes;
	teststats->class_corrects = (long*)my_malloc(sizeof(long)*teststats->num_classes);
	teststats->class_incorrects = (long*)my_malloc(sizeof(long)*teststats->num_classes);
	teststats->class_misses = (long*)my_malloc(sizeof(long)*teststats->num_classes);
	for (i=0; i<teststats->num_classes; i++)
	{
		teststats->class_corrects[i] = 0;
		teststats->class_incorrects[i] = 0;
		teststats->class_misses[i] = 0;
	}
  }

  if (ex.y.class >=1 && ex.y.class <= teststats->num_classes
  	&& ypred.class >=1 && ypred.class <= teststats->num_classes)
  {
	  if (ex.y.class == ypred.class)
	  {
	  	teststats->class_corrects[ex.y.class-1]++;
	  }
	  else
	  {
	  	teststats->class_incorrects[ex.y.class-1]++;
		teststats->class_misses[ypred.class-1]++;
	  }
  }
}
...
void free_struct_testing_stats(STRUCT_TEST_STATS* teststats)
{
	if (teststats->class_corrects)
	{
		free(teststats->class_corrects);
	}
	
	if (teststats->class_incorrects)
	{
		free(teststats->class_incorrects);
	}

	if (teststats->class_misses)
	{
		free(teststats->class_misses);
	}
}
