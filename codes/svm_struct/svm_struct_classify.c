...
int main (int argc, char* argv[])
{
  ...
  if((!no_accuracy) && (verbosity>=1)) {
    ...
    printf("Precision on test set: %.2f%% (%ld correct, %ld incorrect, %d total)\n",(float)100.0*correct/testsample.n,correct,incorrect,testsample.n);

	printf("Precision and relative information on each classes (total %d classes):\n", teststats.num_classes);
	printf("Class(N)  Total(N)  Correct(N)  Incorrect(N)  MissAs(N)  Precision\n");
	for (i=0; i<teststats.num_classes; i++)
	{
		printf("Class%-5.1d%-10.1d%-12.1d%-14.1d%-11.1d%.2f%%\n", i+1, 
  ...
  teststats.runtime = runtime;
  print_struct_testing_stats(predfl, testsample,&model,&sparm,&teststats);
  fclose(predfl);
  
  free_struct_sample(testsample);
  free_struct_model(model);
  
  free_struct_testing_stats(&teststats);
